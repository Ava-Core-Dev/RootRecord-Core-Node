#!/usr/bin/env python3
"""Opt-in safe GitHub auto-push for RootRecord development checkouts."""
...
def main() -> int:
    ...
    try:
        ...
        staged = [x for x in git("diff", "--cached", "--name-only").stdout.splitlines() if allowed(x)]
        denied = [x for x in git("diff", "--cached", "--name-only").stdout.splitlines() if not allowed(x)]
        if denied:
        ...
        message = f"auto: sync {datetime.now().astimezone().strftime('%Y-%m-%d %H:%M %Z')}"
        commit = subprocess.run(["git", "commit", "-m", message], cwd=ROOT, env=env, text=True, capture_output=True, **HIDDEN_RUN)
